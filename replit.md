...
-   **Settings Hub**: Card-based selector layout (matching Reports Hub pattern) with drag-and-drop reordering via `@dnd-kit`. Per-user card order is persisted via `/api/user-preferences/settings-order`. Role-gated card visibility (Admin sees all 13 cards, Internal Designer sees only Input Fields). Deep linking supported via `?tab=` query parameter with role validation. Settings sections: Services, Pricing, Line Items, Bundles, Packs, Subscriptions, Automation, Discounts, Vendors, Priority, Email Preferences, Services Sorting, Input Fields.
-   **Services Sorting (Admin Settings)**: Drag-and-drop interface with 3 tabs (Ad-hoc Services, Bundles, Monthly Packs) that allows admins to reorder how each category appears on the client-facing Services Selection screen. Displays active items as cards in a 3-column grid matching the client view layout. Uses the `displayOrder` column on `services`, `bundles`, and `service_packs` tables. Changes save automatically via bulk update endpoints: `PUT /api/services/sort-order`, `PUT /api/bundles/sort-order`, `PUT /api/service-packs/sort-order`. The order applies globally to all clients.
-   **Admin Email Preferences Matrix**: A Settings section allowing Admins to configure which workflow emails each admin receives. Displays a matrix with admin users as rows and 6 email types as columns (New Service Requests, Job Cancellation, New Pack Subscriptions, Pack Cancellation, Pack Upgrade, Pack Downgrade). Stored in `admin_email_preferences` table with unique (adminId, emailType) constraint. Defaults to enabled. Filtering happens server-side before SendGrid calls; in-app notifications are still sent to all admins regardless of email preferences.
