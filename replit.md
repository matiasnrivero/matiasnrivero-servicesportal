...
-   **Services Sorting (Admin Settings)**: Drag-and-drop interface that allows admins to reorder how services appear on the client-facing Services Selection screen. Displays all active parent services as cards in a 3-column grid matching the client view layout (title, price, description). Uses the `displayOrder` column on the `services` table. Changes save automatically via `PUT /api/services/sort-order` bulk update endpoint. The order applies globally to all clients.
